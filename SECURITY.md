...
- **Replay of a sealed state, across processes.** The state proves _binding_ —
  that this server asked about this operation, for this caller — and, since
  0.8.1, _freshness_ within one process: it carries a nonce that is spent the
  first time an answer arrives with it, accepted or declined, and a second
  presentation produces a fresh question. That record is kept in memory, like
  the key. A restart, or a second process serving the other half of the flow
  with a supplied `key`, has no record — the state opens there as if unseen,
  and a retried leg can run an approved operation once more without asking
  again, until the state expires.

  Whoever can replay a state received the `input_required` themselves, so they
  are the client — and a compromised client is already the first item on this
  list. Still, for anything irreversible or non-idempotent — sending mail,
  issuing a credential, triggering a run — make the operation itself idempotent
  or check before acting, in the server, where the knowledge lives. The
  two-call token has the same shape: a secret the server keeps in memory and
  `consume` deletes on use.
